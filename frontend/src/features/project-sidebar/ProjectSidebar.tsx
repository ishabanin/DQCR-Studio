...
import { useValidationBadges, type ValidationBadgeItem } from "../../shared/hooks/useValidationBadges";
import ProjectStructureDialog, { type ProjectStructureActionMode, type ProjectStructureActionState } from "../../shared/components/ProjectStructureDialog";
import Tooltip from "../../shared/components/ui/Tooltip";

type ActionIconName = "new-file" | "new-folder" | "new-model" | "rename" | "delete" | "collapse-all" | "reveal-active" | "system-folders" | "more";
type NodeVisualKind =
  | "project"
  | "readme"
  | "contexts"
...
function getActionLabel(mode: ProjectStructureActionMode): string {
  switch (mode) {
    case "new-file":
      return "Новый файл";
    case "new-folder":
...
function SidebarTreeNode({
  ...
  const availableActionModes = getAvailableActionModes(node.path, node.type);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {availableActionModes.length > 0 ? (
          <div className={`tree-row-actions tree-row-actions-single ${menuOpen ? "tree-row-actions-open" : ""}`} ref={menuRef}>
            <TreeActionButton icon="more" label="Действия" onClick={() => setMenuOpen((previous) => !previous)} />
            {menuOpen ? (
              <div className="tree-node-menu" role="menu" aria-label={`Действия для ${node.name}`}>
                {availableActionModes.map((mode) => (
                  <button
                    key={mode}
                    type="button"
                    ...
                    onClick={(event) => {
                      event.stopPropagation();
                      setMenuOpen(false);
                      onAction(mode, node.path, node.type);
                    }}
                  >
                    <span className="tree-node-menu-icon" aria-hidden="true">
                      <ActionGlyph
                        name={
                          mode === "new-file"
                            ? "new-file"
                    ...
                    <span className="tree-node-menu-text">{getActionLabel(mode)}</span>
                  </button>
                ))}
              </div>
            ) : null}
