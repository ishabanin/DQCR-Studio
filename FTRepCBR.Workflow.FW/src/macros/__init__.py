...
class MacroRegistry:
    ...
    def _load(self):
        ...
        for tool_name in self._tools:
            ...
            if tool_dir.exists():
                self._load_macros_from_dir(tool_dir, tool=tool_name, base_path=tool_name)
        ...
        for engine_name in self._workflow_engines:
            ...
            if engine_dir.exists():
                self._load_macros_from_dir(engine_dir, tool=engine_name, base_path=f"workflow/{engine_name}")
        ...
        if self._custom_path and self._custom_path.exists():
            ...
            if custom_main.exists():
                self._load_macros_from_dir(custom_main, tool=None, base_path="main", override=True)
            
            # tools
            for tool_name in self._tools:
                custom_tool = self._custom_path / tool_name
                if custom_tool.exists():
                    self._load_macros_from_dir(custom_tool, tool=tool_name, base_path=tool_name, override=True)
            
            # workflow engines
            for engine_name in self._workflow_engines:
                custom_engine = self._custom_path / "workflow" / engine_name
                if custom_engine.exists():
                    self._load_macros_from_dir(custom_engine, tool=engine_name, base_path=f"workflow/{engine_name}", override=True)
        ...
        logger.info(f"Loaded {len(self._macros)} macros, {len(self._python_macros)} python macros")
    
    def _load_python_macros(self):
        """Загрузить Python-макросы из всех директорий."""
        
        def load_from_dir(directory: Path, tool: Optional[str] = None, override: bool = False):
            ...
            for py_file in directory.rglob("*.py"):
                ...
                try:
                    spec = importlib.util.spec_from_file_location(f"macro_{py_file.stem}", py_file)
                    ...
                    if func:
                        ...
                        if override or key not in self._python_macros:
                            ...
                            action = "Overridden" if key in self._python_macros else "Loaded"
                            logger.debug(f"{action} python macro: {key}")
                
                except Exception as e:
                    logger.warning(f"Error loading python macro from {py_file}: {e}")
        
        def load_workflow_macro(engine_dir: Path, engine_name: str, override: bool = False):
            ...
            try:
                spec = importlib.util.spec_from_file_location(f"workflow_{engine_name}", init_file)
                if spec is None or spec.loader is None:
                    return
    ...
    def _get_python_macro_name(self, py_file: Path, base_dir: Path, tool: Optional[str]) -> str:
    ...
    def _load_workflow_templates(self):
        ...
        if self._custom_path and self._custom_path.exists():
            for engine_name in self._workflow_engines:
                custom_templates = self._custom_path / "workflow" / engine_name / "templates"
...
class FunctionRegistry:
    ...
    def __init__(self, tools: Optional[List[str]] = None, macros_path: Optional[Path] = None):
    ...
    def _load_functions_from_file(self, file_path: Path, tool: Optional[str] = None) -> Dict[str, Callable]:
        ...
        try:
            spec = importlib.util.spec_from_file_location(f"func_{file_path.stem}", file_path)
            if spec is None or spec.loader is None:
                return functions
    ...
    def _register_all_functions(self):
        ...
        if funcs_dir.exists():
            for py_file in funcs_dir.glob("*.py"):
                if py_file.stem == "__init__":
                    continue
                self._base_functions.update(self._load_functions_from_file(py_file, tool=None))
        ...
        self._base_functions.update({
            ...
            'iff': lambda cond, true_val, false_val: true_val if cond else false_val,
        ...
        logger.debug(f"Loaded {len(self._base_functions)} base functions, "
...
def get_function_registry(tools: Optional[List[str]] = None, macros_path: Optional[Path] = None) -> FunctionRegistry:
