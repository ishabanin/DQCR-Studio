    ...
    def from_dict(data: Any) -> "ParameterValue":
        ...
        if isinstance(data, dict):
            return ParameterValue(
                type=data.get("type", "static"), value=data.get("value")
            )
...
class ParameterModel:
    ...
    Атрибуты:
        ...
        config: конфиг в разрезе контекстов {context: {property: ConfigValue}}
        metadata: метаданные из парсинга SQL
        compiled: скомпилированные данные {context: {tool: {field: value}}}
        generated: флаг, что параметр сгенерирован
        project_props: ссылки на свойства проекта (_p.props.*)
        context_flags: ссылки на флаги контекста (_ctx.flags.*)
        context_constants: ссылки на константы контекста (_ctx.const.*)
    """
    ...
    def from_dict(name: str, data: Dict[str, Any]) -> "ParameterModel":
        ...
        domain_type = param_data.get("domain_type")
        description = param_data.get("description", "")
        attributes = param_data.get("attributes", [])
        param_type = param_data.get("param_type", "variable")
        if not attributes:
            attributes = [{"name": name, "domain_type": domain_type or "UNDEFINED"}]
