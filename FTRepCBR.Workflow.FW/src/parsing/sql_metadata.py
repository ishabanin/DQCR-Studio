...
SQL_KEYWORDS = {
    ...
    'VALUES', 'INSERT', 'UPDATE', 'DELETE', 'SET', 'INTO',
    'CREATE', 'ALTER', 'DROP', 'TABLE', 'INDEX', 'VIEW',
    'PRIMARY', 'KEY', 'FOREIGN', 'REFERENCES', 'CONSTRAINT',
    'DEFAULT', 'CHECK', 'UNIQUE', 'CASCADE',
    'GRANT', 'REVOKE', 'COMMIT', 'ROLLBACK', 'SAVEPOINT',
    'FETCH', 'NEXT', 'ONLY', 'PERCENT', 'TIES',
    'FOR', 'OF', 'NO', 'NOWAIT', 'SKIP', 'LOCKED',
    'LATERAL', 'NATURAL', 'USING', 'MATERIALIZED', 'EXTEND',
    'MINUS', 'INTERSECT', 'SUBMULTISET', 'MULTISET',
}
...
class SQLMetadataParser:
    ...
    CTE_SOURCE_CTE_PATTERN = re.compile(r'\bfrom\s+([a-zA-Z_][a-zA-Z0-9_]*)', re.IGNORECASE)
    ...
    def extract_tables(self, sql_content: str, cte_names: Set[str] = None) -> Dict[str, Dict[str, Any]]:
        """Извлечь таблицы."""
        tables = {}
        ...
        for match in self.FROM_PATTERN.finditer(sql_content):
            ...
            if alias and (not alias[0].isalpha() and alias[0] != '_' or alias.upper() in SQL_KEYWORDS):
                alias = None
            
            if var_table:
                table_name = var_table.replace('{{', '').replace('}}', '').strip()
                tables[table_name] = {'alias': (alias or table_name).lower(), 'is_variable': True, 'is_cte': False}
            elif literal_table:
                is_cte = literal_table.lower() in {c.lower() for c in cte_names}
                tables[literal_table] = {'alias': (alias or literal_table).lower(), 'is_variable': False, 'is_cte': is_cte}
    ...
    def extract_cte(self, sql_content: str) -> Dict[str, Dict[str, Any]]:
        ...
        for i, name_match in enumerate(cte_def_matches):
            ...
            match = re.search(r'\b[a-zA-Z0-9_]+\s+as\s*\((.+)\)', cte_block, re.IGNORECASE | re.DOTALL)
            ...
            if sql_content[i] == '(':
                ...
                if depth == 0:
                    while i + 1 < len(sql_content) and sql_content[i + 1].isspace():
                        i += 1
                if i + 1 < len(sql_content) and sql_content[i + 1:i + 7].upper() == 'SELECT':
    ...
    def extract_field_aliases(self, sql_content: str, known_aliases: Set[str]) -> List[Dict[str, str]]:
        ...
        if search_start > 0:
            select_content_match = self.SELECT_FIELD_PATTERN.search(sql_content[search_start:])
        else:
        ...
        as_pattern = re.compile(r'^(.+?)\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*)$', re.IGNORECASE)
        implicit_pattern = re.compile(r'^([a-zA-Z_][a-zA-Z0-9_]*)\.([a-zA-Z_][a-zA-Z0-9_]*)$', re.IGNORECASE)
        ...
        for field_expr in fields:
            ...
            if alias_name:
                aliases.append({'alias': alias_name, 'source': source, 'expression': expression})
