...
from FW.models import WorkflowConfig, TargetTableModelNew, FolderConfig
from FW.models.attribute import Attribute
from FW.models.workflow_new import CTEMaterializationConfig
...
def load_target_table(model_path: Path, default_name: str = "") -> TargetTableModelNew:
...
def load_folder_configs(
    model_path: Path, sql_folder_name: str = "SQL"
        ...
        try:
            ...
            for folder_name, folder_data in data.items():
                if not isinstance(folder_data, dict):
                    logger.warning(
                        f"Invalid folder config in {folder_yml}: expected dict, got {type(folder_data)}"
                    )
                    continue

                folder_config = FolderConfig.from_dict(folder_data)

                folder_configs[folder_name] = folder_config
                logger.info(
                    f"Loaded folder config from {folder_yml} for folder '{folder_name}'"
                )
...
def merge_workflow_configs(
    base_config: WorkflowConfig, folder_configs: Dict[str, FolderConfig]
) -> WorkflowConfig:
